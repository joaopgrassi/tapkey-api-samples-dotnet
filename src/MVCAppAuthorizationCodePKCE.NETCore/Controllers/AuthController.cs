    ...
    public class AuthController : Controller
    {
        ...
        public async Task<IActionResult> LoginCallback([FromQuery]string code)
        {
            ...
            var tokenRequest = new Dictionary<string, string>
            {
                ...
                // you can specify the scopes again (also fewer), but not scopes that weren't initially authorized.
                // if not provided, all scopes previously requested will be returned.
                {"scope", "read:core:entities read:owneraccounts"},
            };
            ...
            var refreshTokenRequest = new Dictionary<string, string>
            {
                ...
                // Need to pass the refresh token obtained from the token endpoint (if requested the scope offline_access)
                // Tapkey allows only one refresh token per access_token. 
